...
class Board:
    ...
    def print_moves(self):
        ...
        for m in self.moves:
            ...
            capture = ascii_pieces[not self.side][capture]
    ...
    def get_moves(self):
        ...
        bishopqueen_checkmask = get_bishop_attacks(king_sq, both_occupancy) & opp_bishopqueen_bb
        rookqueen_checkmask = get_rook_attacks(king_sq, both_occupancy) & opp_rookqueen_bb
        check_count = (pawn_checkmask >  0) + (knight_checkmask > 0) + (bishopqueen_checkmask > 0) + (rookqueen_checkmask > 0)
        ...
        # double check
            ...
            attacks = get_king_attacks(king_sq) & flip_bit(occupancy)
            for target_sq in get_set_bits_idx(self.get_non_attacked_bb(attacks,  not self.side)):
                if 1 << target_sq & opp_occupancy:
                    self.moves.append((king_sq, target_sq, K, 0, 1, 0, 0, 0))
                else:
                    self.moves.append((king_sq, target_sq, K, 0, 0, 0, 0, 0))
            return
        

        # GENERATE PINMASK 
        orthogonal_pinmask = 0
        orthogonal_pinner = get_xray_rook_attacks(king_sq, occupancy, both_occupancy) & opp_rookqueen_bb
        for pinner in get_set_bits_idx(orthogonal_pinner):
            pinmask = RECT_LOOKUP[pinner][king_sq]
            orthogonal_pinmask |= pinmask
        diagonal_pinmask = 0
        diagonal_pinner = get_xray_bishop_attacks(king_sq, occupancy, both_occupancy) & opp_bishopqueen_bb
        ...
        if self.side == WHITE:
            ...
            for target_sq in get_set_bits_idx(single_push_target_bb & not_8_rank):
                self.moves.append((target_sq + 8, target_sq, P, 0, 0, 0, 0, 0))
            # single pawn pushes with promotion
            for target_sq in get_set_bits_idx(single_push_target_bb & rank_8):
                self.moves.append((target_sq + 8, target_sq, P, Q, 0, 0, 0, 0))
                self.moves.append((target_sq + 8, target_sq, P, R, 0, 0, 0, 0))
                self.moves.append((target_sq + 8, target_sq, P, B, 0, 0, 0, 0))
                self.moves.append((target_sq + 8, target_sq, P, N, 0, 0, 0, 0))
            
            double_push_pawn_bb = single_push_pawn_bb & rank_2
            double_push_target_bb = (double_push_pawn_bb >> 8 & flip_bit(both_occupancy)) >> 8 & flip_bit(both_occupancy) & checkmask
            # double pawn pushes
            for target_sq in get_set_bits_idx(double_push_target_bb):
                self.moves.append((target_sq + 16, target_sq, P, 0, 0, 1, 0, 0))
            ...
            for target_sq in get_set_bits_idx(right_capture_target_bb & not_8_rank):
                self.moves.append((target_sq + 7, target_sq, P, 0, 1, 0, 0, 0))
            # captures  to the right with promotion
            for target_sq in get_set_bits_idx(right_capture_target_bb & rank_8):
                self.moves.append((target_sq + 7, target_sq, P, Q, 1,  0, 0, 0))
                self.moves.append((target_sq + 7, target_sq, P, N, 1,  0, 0, 0))
                self.moves.append((target_sq + 7, target_sq, P, B, 1,  0, 0, 0))
                self.moves.append((target_sq + 7, target_sq, P, R, 1,  0, 0, 0))
            ...
            for target_sq in get_set_bits_idx(left_capture_target_bb & not_8_rank):
                self.moves.append((target_sq + 9, target_sq, P, 0, 1, 0, 0, 0))
            # captures to the left with promotion
            for target_sq in get_set_bits_idx(left_capture_target_bb & rank_8):
                self.moves.append((target_sq + 9, target_sq, P, Q, 1, 0, 0, 0))
                self.moves.append((target_sq + 9, target_sq, P, N, 1, 0, 0, 0))
                self.moves.append((target_sq + 9, target_sq, P, B, 1, 0, 0, 0))
                self.moves.append((target_sq + 9, target_sq, P, R, 1, 0, 0, 0))
            if self.enpassant:
                enpassant_source_bb = get_pawn_attacks(not self.side, self.enpassant) & pawn_bb
                for source_sq in get_set_bits_idx(enpassant_source_bb):
                    new_both_occupancy = both_occupancy ^ ((1 << source_sq)  | (1 << self.enpassant + 8))
                    if not get_rook_attacks(king_sq, new_both_occupancy) & opp_rookqueen_bb and not get_bishop_attacks(king_sq, new_both_occupancy) & opp_bishopqueen_bb:
                        self.moves.append((source_sq,  self.enpassant, P, 0, 0, 0, 1, 0))
            ...
            # squares between king's rook and king are unoccupied
                if not both_occupancy & 6917529027641081856:
                    ...
                    if not self.is_square_attacked(f1, not self.side) and not self.is_square_attacked(g1, not self.side) and not self.is_square_attacked(king_sq, not self.side):
                         self.moves.append((e1, g1, K, 0, 0, 0, 0, 1))
            # queenside castle
            if self.castle & wq:
                # squares between queen's rook and king are unoccupied
                if not both_occupancy & 2017612633061982208:
                    # squares between queen's rook and king are not under attack
                    if not self.is_square_attacked(b1, not self.side) and not self.is_square_attacked(c1,not self.side) and not self.is_square_attacked(d1, not self.side) and not self.is_square_attacked(king_sq, not self.side):
                        self.moves.append((e1, c1, K, 0, 0, 0, 0, 1))
        ...
        # self.side == BLACK
            ...
            double_push_target_bb = double_push_pawn_bb << 16 & flip_bit(both_occupancy) & checkmask
            # double pawn pushes
            for target_sq in get_set_bits_idx(double_push_target_bb):
                self.moves.append((target_sq - 16, target_sq, P, 0, 0, 1, 0, 0))
            ...
            # squares between king's rook and king are unoccupied
                if not both_occupancy & 96:
                    # f and g squares between king's rook and king are not under attack
                    if not self.is_square_attacked(f8, not self.side) and not self.is_square_attacked(g8, not self.side) and not self.is_square_attacked(king_sq, not self.side):
                         self.moves.append((e8, g8, K, 0, 0, 0, 0, 1))
            # queenside castle
            if self.castle & bq:
                # squares between queen's rook and king are unoccupied
                if not both_occupancy & 28:
                    # squares between queen's rook and king are not under attack
                    if not self.is_square_attacked(b8, not self.side) and not self.is_square_attacked(c8,not self.side) and not self.is_square_attacked(d8, not self.side) and not self.is_square_attacked(king_sq, not self.side):
                        self.moves.append((e8, c8, K, 0, 0, 0, 0, 1))
        ...
        '''
        # GENERATE SILENT PAWN MOVES
        for source_sq in get_set_bits_idx(pawn_bb & flip_bit(diagonal_pinmask)):
            target_sq = source_sq + pawn_dir
            # Check that no pieces are blocking pawn and move is within confines of checkmask
            if (1 << source_sq) & orthogonal_pinmask  and not (1 << target_sq) & orthogonal_pinmask: 
                continue
            if (1 << target_sq) & flip_bit(both_occupancy) & checkmask:
                ...
                if min_promo_sq <= source_sq <= max_promo_sq:
                    ...
                    # Double pawn pushes
                    double_target_sq = target_sq + pawn_dir
                    if min_double_sq <= source_sq <= max_double_sq and (1 << double_target_sq) & flip_bit(both_occupancy) & checkmask:
                        self.moves.append((source_sq, target_sq + pawn_dir, P, -1, -1, 1, 0, 0))
                        
        # GENERATE PAWN CAPTURES
        for source_sq in get_set_bits_idx(pawn_bb & flip_bit(diagonal_pinmask)):
            attacks = get_pawn_attacks(self.side, source_sq) & opp_occupancy & checkmask
        ...
        # EN PASSANT
        enpassant_source_sq = get_pawn_attacks(not self.side, self.enpassant) & pawn_bb
        ...
        # KNIGHT MOVES
        for source_sq in get_set_bits_idx(knight_bb & flip_bit(orthogonal_pinmask | diagonal_pinmask)):
            attacks = get_knight_attacks(source_sq) & checkmask & flip_bit(occupancy)
            # captures
            for target_sq in get_set_bits_idx(attacks & opp_occupancy):
                self.moves.append((source_sq, target_sq, N, 0, 1, 0, 0, 0))
            # quiet moves
            for target_sq in get_set_bits_idx(attacks & flip_bit(both_occupancy)):
                self.moves.append((source_sq, target_sq, N, 0, 0, 0, 0, 0))
        ...
        for source_sq in get_set_bits_idx(bishop_bb & flip_bit(orthogonal_pinmask)):
            ...
            # captures
            for target_sq in get_set_bits_idx(attacks & opp_occupancy):
                self.moves.append((source_sq, target_sq, B, 0, 1, 0, 0, 0))
            # quiet  moves
            for target_sq in get_set_bits_idx(attacks & flip_bit(both_occupancy)):
                self.moves.append((source_sq, target_sq, B, 0, 0, 0, 0, 0))
        ...
        for source_sq in get_set_bits_idx(rook_bb & flip_bit(diagonal_pinmask)):
            ...
            # captures
            for target_sq in get_set_bits_idx(attacks & opp_occupancy):
                self.moves.append((source_sq, target_sq, R, 0, 1, 0, 0, 0))
            # quiet moves
            for target_sq in get_set_bits_idx(attacks & flip_bit(both_occupancy)):
                self.moves.append((source_sq, target_sq, R, 0, 0, 0, 0, 0))
        ...
        for source_sq in get_set_bits_idx(queen_bb):
            ...
            elif source_bb & nwse_pinmask:
                attacks &= nwse_pinmask
            # captures
            for target_sq in get_set_bits_idx(attacks & opp_occupancy):
                self.moves.append((source_sq, target_sq, Q, 0, 0, 0, 0, 0))
            # quiet moves
            for target_sq in get_set_bits_idx(attacks & flip_bit(both_occupancy)):
                self.moves.append((source_sq, target_sq, Q, 0, 0, 0, 0, 0))
        ...
        for piece, func in zip(attack_pieces, attack_funcs):
            ...
            while bb:
                ...
                while attacks:
                    ...
                    attacks = pop_bit(attacks, target_sq)
                bb = pop_bit(bb, source_sq)'''
    def perft(self, max_depth):
        
    def perft_driver(self, depth):
        if depth == 0:
            PERFT_NODES += 1
        self.get_moves()
        self.make_move(self.moves.pop())
