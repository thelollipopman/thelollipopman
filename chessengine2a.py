...
class Board:
    ...
    def print_board(self):
        ...
        print('    Enpass: {}'.format(
            sq_to_coord[self.enpassant_states[-1]] if self.enpassant_states[-1] != -1 else None))
    ...
    def is_square_attacked(self, square, attacking_side):
        ...
        if get_pawn_attacks(not attacking_side, square) & pawn_bb:
            return 1
    ...
    def get_moves(self):
        ...
        if self.side == WHITE:
            ...
            if self.enpassant:
                enpassant_source_bb = get_pawn_attacks(not self.side, self.enpassant) & pawn_bb & flip_bit(orthogonal_pinmask)
            ...
            if self.castle & wk:
                ...
                if not both_occupancy & 6917529027641081856:
                    ...
                    if not self.is_square_attacked(f1, not self.side) and not self.is_square_attacked(g1,
                                                                                                      not self.side) and not self.is_square_attacked(
                            king_sq, not self.side):
            ...
            if self.castle & wq:
                ...
                if not both_occupancy & 1008806316530991104 :
                    ...
                    if not self.is_square_attacked(b1, not self.side) and not self.is_square_attacked(c1,
                                                                                                      not self.side) and not self.is_square_attacked(
                            d1, not self.side) and not self.is_square_attacked(king_sq, not self.side):
        ...
        # self.side == BLACK
            ...
            if self.castle & bk:
                ...
                if not both_occupancy & 96:
                    # f and g squares between king's rook and king are not under attack
                    if not self.is_square_attacked(f8, not self.side) and not self.is_square_attacked(g8,
                                                                                                      not self.side) and not self.is_square_attacked(
                            king_sq, not self.side):
            ...
            if self.castle & bq:
                ...
                if not both_occupancy & 14:
                    # squares between queen's rook and king are not under attack
                    if not self.is_square_attacked(b8, not self.side) and not self.is_square_attacked(c8,
                                                                                                      not self.side) and not self.is_square_attacked(
                            d8, not self.side) and not self.is_square_attacked(king_sq, not self.side):
    ...
    def perft(self, max_depth):
        ...
        print(f'Nodes: {self.perft_nodes}')
        return self.perft_nodes
    ...
    def perft_divide(self, depth):
        perft_divide = {}
        for move in self.get_moves():
            uci_move = f'{sq_to_coord[move[0]]}{sq_to_coord[move[1]]}'
            self.make_move(move)
            perft_divide[uci_move] = self.perft(depth - 1)
            self.unmake_move()
        return perft_divide
    ...
    def perft_divide(self, max_depth):
        perft_divide = {}
        for move in self.get_moves():
            self.make_move(move)
            self.perft(max_depth - 1)
            perft_divide[move] = self.perft_nodes
            self.unmake_move()
        return perft_divide
    def debug_perft_dri(self, depth, max_depth):
        if depth == max_depth:
            return 1
        nodes = 0
        for move in self.get_moves():
            self.make_move()
            nodes += self.debug_perft_driver(depth + 1, max_depth)
            self.unmake_move()
...
def sf_perft_divide(fen, depth):
    ...
    process = Popen(stockfish_path, stdin=PIPE, stdout=PIPE, stderr=PIPE, text=True)
    command = f'''
position fen {fen}
go perft {depth}
'''
...
def debug_perft(start_fen, depth):
    board = Board()
    board.parse_fen(start_fen)
    while True:
        fen = board.get_fen()
        board_perft_div = board.perft_divide(depth)
        sf_perft_div = sf_perft_divide(fen, depth)
        board_perft_div_keys = set(f'{sq_to_coord[move[0]]}{sq_to_coord[move[1]]}' for move in board_perft_div.keys())
        if board_perft_div_keys != set(sf_perft_div.keys()):
            print(f'Error')
            print(fen)
            return fen
        no_error_flag = True
        for move, move_count in board_perft_div.items():
            uci_move = f'{sq_to_coord[move[0]]}{sq_to_coord[move[1]]}'
            if sf_perft_div[uci_move] != move_count:
                board.make_move(move)
                depth -= 1
                no_error_flag = False
                break
        if no_error_flag == True:
            print('No errors')
            break
#debug_perft(start_pos, 4)
'rnbqkbnr/1ppppppp/p7/8/P7/8/1PPPPPPP/RNBQKBNR b KQkq - 0 1'
