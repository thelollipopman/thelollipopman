...
def mask_king_attacks(square):
    ...
    if king & not_8_rank:
        attacks |= (king >> 8)
    if king & not_a_file and king & not_8_rank:
        attacks |= (king >> 9)
    if king & not_a_file and king & not_1_rank:
        attacks |= (king << 7)
    if king & not_h_file and king & not_8_rank:
        attacks |= (king >> 7)
    if king & not_h_file and king & not_1_rank:
        attacks |= (king << 9)
    return attacks
...
class Board:
        ...
        if self.side == WHITE:
            ...
            if self.castle & wq:
                ...
                if not both_occupancy & 1008806316530991104 :
                    ...
                    if not self.is_square_attacked(c1, not self.side) and not self.is_square_attacked(d1, not self.side) and not self.is_square_attacked(king_sq, not self.side):
        ...
        # self.side == BLACK
            ...
            if self.castle & bq:
                ...
                if not both_occupancy & 14:
                    # squares between queen's rook and king are not under attack
                    if not self.is_square_attacked(c8, not self.side) and not self.is_square_attacked(d8, not self.side) and not self.is_square_attacked(king_sq, not self.side):
    ...
    def perft(self, max_depth, print_flag=True):
        ...
        nodes, timing = self.perft_nodes, end - start
        if print_flag:
            print(f'Nodes: {nodes}')
            print(f'Time: {timing}')
        return nodes, timing
...
def sf_perft_divide(fen, depth):
    process = Popen('pythonscripts/stockfish_14.1_android_armv7/stockfish_14.1_android_armv7/stockfish.android.armv7', stdin=PIPE, stdout=PIPE, stderr=PIPE, text=True)
...
def debug_perft(fen, depth):
    ...
    while board_pdiv != sf_pdiv:
        ...
        for idx,(move, move_count) in enumerate(board_pdiv.items()):
            if sf_perft_div[move] != move_count:
                move = self.get_moves()[idx]
...
'/storage/emulated/0/Android/data/ru.iiec.pydroid3/files/pythonscripts/stockfish_14.1_android_armv7/stockfish_14.1_android_armv7/stockfish.android.armv7'
'/storage/emulated/0/Android/data/ru.iiec.pydroid3/files'

'/storage/emulated/0/Android/data/ru.iiec.pydroid3/files/pythonscripts/stockfish.android.armv7'
