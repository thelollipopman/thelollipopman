...
class Board:
    def __init__(self):
        self.bitboards = [[71776119061217280, 2594073385365405696, 4755801206503243776, 9295429630892703744, 1152921504606846976, 576460752303423488], [65280, 36, 66, 129, 16, 8]]
    ...
    def iterative_deepening_search(self, depth, print_flag=True):
        self.reset_var()
        infos = []
        for current_depth in range(depth):
            info = negamax_search(current_depth, print_flag=False, reset_flag=False)
            infos.append(info)
        if print_flag == True:
            for info in infos:
                print('info cp {cp} depth {depth} nodes {nodes} pv {pv}'.format(**info))
            print('bestmove {}'.format(infos[-1]['bestmove']))
        return infos
            
            
            

    def negamax_search(self, depth, print_flag=True, reset_flag=True):
        if reset_flag == True:
            self.reset_var()
        start_time = time.time()
        end_time = time.time()
        score = self.negamax_driver2(float('-inf'), float('inf'), depth)
        self.pv_nodes = [to_uci(move) for move in self.pv_table[0][:self.pv_length[0]]]
        best_move = self.pv[0]
        pv = ' '.join(pv)
        info = {
            'bestmove': best_move,
