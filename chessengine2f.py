...
class Board:
    ...
    def iterative_deepening_search(self, depth, print_flag=True):
        ...
        for current_depth in range(1, depth + 1):
            self.follow_pv = True
            info = self.negamax_search(current_depth, print_flag=False)
            infos.append(info)
        if print_flag == True:
    ...
    def negamax_search(self, depth, print_flag=True):
        ...
        if len(pv) == 0:
            print(self.pv_table[0])
            print(self.pv_length[0])
        info = {
            'bestmove': pv[0],
            'depth': depth,
            'nodes': self.nodes,
    ...
    def negamax_driver2(self, alpha, beta, depth):
        ...
        if depth >= 3 and in_check == False and self.ply > 0:
            self.side = not self.side
            self.enpassant == 0
            value = - self.negamax_driver2(-beta, -beta + 1, depth - 1 - NMP_REDUCTION)
            self.side = not self.side
            if value >= beta:
                return beta
        move_count = 0
